...
stage_height = stage_size[1] # 스테이지의 높이 위에 캐릭터를 놓기 위해 사용
    ...
    for ball_index, ball_val in enumerate(balls):
        ...
        for weapon_index, weapon_val in enumerate(weapons):
            ...
            if weapon_rect.colliderect(ball_rect):
                weapon_to_remove = weapon_index # 해당 무기 없에기 위한 값 설정
                ball_to_remove = ball_index # 해당 공 없에기 위한 값 설정
                ...
                if ball_img_idx < 3:
                    ...
                    balls.append({
                        ...
                        "to_x" : 3, # x축 이동방향, -3 이면 왼쪽으로, 3 이면 오른쪽으로
                        "to_y" : -6, # y축 이동방향,
                        "init_spd_y" : ball_speed_y[ball_img_idx + 1] }) # y 최초 속 도  
                break
        else: # 계속 게임을 진행
